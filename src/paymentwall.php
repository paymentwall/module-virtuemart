...
if (!class_exists('Paymentwall_Config'))
    require(VMPATH_PLUGINS . DS . 'vmpayment' . DS . 'paymentwall' . DS . 'paymentwall-php' . DS . 'lib' . DS . 'paymentwall.php');
...
class plgVmpaymentPaymentwall extends vmPSPlugin
{
    ...
    function plgVmConfirmedOrder($cart, $order)
    {
        ...
        $widget = new Paymentwall_Widget(
            ...
            array(                                                  // product details for Flexible Widget Call. To let users select the product on Paymentwall's end, leave this array empty
                new Paymentwall_Product(
                    ...
                    $this->getCurrencyCodeById($order['details']['BT']->order_currency), // currency code
                    'Order #' . strtoupper($order['details']['BT']->order_number),                     // product name
        ...
        vRequest::setVar('html', $widget->getHtmlCode(array(
            'frameborder' => '0',
            'width' => '100%',
            'height' => '600'
        )));
     ...
     * @param $payment_id
     */
    public function initPaymentwallConfigs($payment_id)
    {
        if ($params = $this->getPaymentConfigs($payment_id)) {
            Paymentwall_Config::getInstance()->set(array(
                'api_type' => Paymentwall_Config::API_GOODS,
                'public_key' => $params['app_key'],
